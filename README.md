...
OpenMRS frontend configuration files are JSON files containing module names as top-level
elements. All configuration elements are optional. The available configuration elements
for each module should be documented in the module's wiki page.
...
### How do I find out what configuration values are available?

Configuration values should be documented in the module's README. If they're not,
please do prod the maintainer(s) about it.

There is one piece of configuration that is not module-specific. It's called
`common`, and has the following schema:
...
You can configure these values like so:

```json
{
  "common": {
    "concepts": {
      "pulseUuid": "437cfd1f-3510-4e82-ac6c-ee5c8a4a7b72"
    }
  }
}
```
...
_NOTE: Due to architecture changes introduced by
[RFC 26](https://github.com/openmrs/openmrs-rfc-frontend/blob/master/text/0026-activation-distribution.md),
this is no longer working as described here. Sorry about that. We're working on getting
this working in the new system._
...
If your module is configurable, you need to document its schema in your module's README.

Consider using [@openmrs/esm-cli](https://github.com/openmrs/openmrs-esm-cli)'s
`config-autodoc` to facilitate this. It's a work in progress and you might run into
problems. Please contact the maintainer or file an issue if you do!
...
Some configuration is shared and available to all modules. Get it with

```js
const config = await useCommonConfig();
```

or in a React component with

```js
const confg = useCommonConfig();
```

It's schema is defined in this module, and is described above.
